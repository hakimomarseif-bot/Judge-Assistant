...
# Go up two directory levels from the script's location to reach the "Code" root
...
if __name__ == "__main__":
    ...
    # أضف هذا المتغير في بداية الملف لتسهيل التحكم
    DATA_DIR = r"D:\FUCK!!\Grad\Backup\1st Iteration\1\Docs" # ضع المسار الحقيقي لمجلد الملفات هنا

    _DOC_MAP = {
        f"{DATA_DIR}/صحيفة_دعوى.txt":               "صحيفة دعوى",
        f"{DATA_DIR}/محضر_جلسة_25_03_2024.txt":      "محضر جلسة",
        f"{DATA_DIR}/تقرير_الخبير.txt":              "تقرير خبير",
        f"{DATA_DIR}/تقرير_الطب_الشرعي.txt":        "تقرير الطب الشرعي",
        f"{DATA_DIR}/حكم_المحكمة.txt":               "حكم",
        f"{DATA_DIR}/مذكرة_بدفاع_المدعى_عليه_الأول.txt":    "مذكرة بدفاع",
        f"{DATA_DIR}/مذكرة_بدفاع_المدعى_عليها_الثانية.txt": "مذكرة بدفاع",
    }

    # 10 evaluation test cases based on real file content
    TEST_CASES = [
        
        {
            "query": "من هم أطراف النزاع في هذه القضية؟",
            "expected_keywords": ["محمد أحمد إبراهيم", "حسام الدين عبد العزيز", "فاطمة حسن عبد الله", "شركة النيل"],
            "ground_truth": "المدعي: محمد أحمد إبراهيم الشرقاوي. المدعى عليه الأول: حسام الدين عبد العزيز محمود. المدعى عليها الثانية: فاطمة حسن عبد الله الجندي. المدعى عليها الثالثة: شركة النيل للاستثمار العقاري.",
        },
        {
            "query": "ما هي طلبات المدعي بخصوص المدعى عليه الأول (حسام الدين)؟",
            "expected_keywords": ["فسخ عقد الإيجار", "إخلاء", "513,835", "تعديلات إنشائية"],
            "ground_truth": "فسخ عقد الإيجار المؤرخ 03/05/2020، إخلاء العين، إلزام بسداد 513,835 جنيه متأخرات إيجارية، وتعويض 1.5 مليون جنيه عن الأضرار الإنشائية.",
        },
        {
            "query": "ماذا انتهى إليه تقرير الخبير الهندسي بشأن أعمال شركة النيل للاستثمار؟",
            "expected_keywords": ["مخالفة للمواصفات", "35-40%", "الخرسانة", "إزالة"],
            "ground_truth": "انتهى التقرير إلى أن أعمال التجديد مخالفة للمواصفات، ونسبة الإنجاز الفعلية تتراوح بين 35-40% فقط، وأن المقاومة الخرسانية أقل من المطلوب بنسبة 22-30% في عدة مواقع.",
        },
        {
            "query": "ما هو رأي الطب الشرعي في التوقيع المنسوب للمرحوم عبد الفتاح الشرقاوي؟",
            "expected_keywords": ["توقيع مقلد", "72-78%", "يناير 2022", "مذيبات"],
            "ground_truth": "أكد التقرير أن التوقيع مقلد (Simulated) وليس صحيحاً، حيث انخفضت نسبة التشابه إلى 72-78%، كما أن العمر الزمني للحبر يعود لعام 2022 بينما العقد مؤرخ في 2019.",
        },
        {
            "query": "ما هي الدفوع التي أبداها المدعى عليه الأول في مذكرته؟",
            "expected_keywords": ["حبس الإيجار", "تسريب مياه", "تخزين مؤقت", "1,200,000"],
            "ground_truth": "دفع بحقه في حبس الإيجار لوجود تسريبات مياه جسيمة، ونفى التأجير من الباطن واصفاً إياه بـ 'تخزين مؤقت'، وطالب في دعوى فرعية بتعويض 1.2 مليون جنيه.",
        },
        {
            "query": "ما هي القيمة الإجمالية للأضرار التي قدرها الخبير الهندسي؟",
            "expected_keywords": ["470,000", "تلفيات", "إصلاح"],
            "ground_truth": "قدر الخبير قيمة التلفيات الإنشائية والمعمارية بمبلغ إجمالي قدره 470,000 جنيه مصري.",
        },
        {
            "query": "بكم اشترت فاطمة حسن الجندي العقار وفقاً لادعائها؟",
            "expected_keywords": ["2,400,000", "1,800,000", "إيصالات"],
            "ground_truth": "ادعت شراء الدورين الثالث والرابع بمبلغ إجمالي 2,400,000 جنيه، سددت منها 1,800,000 جنيه بموجب 5 إيصالات سداد.",
        },
    ]
